...
if [  "$TRAVIS_BRANCH" = "$DEV_BRANCH" ] || [ "$TRAVIS_BRANCH" = "$MASTER_BRANCH" ]; then
  ...
  if [ "$TRAVIS_PULL_REQUEST" = false ]; then
    ...
    if [ -z "$TRAVIS_TAG" ]; then
      ...
      jarsigner -verbose -sigalg SHA1withRSA -storepass $storepass -keypass $keypass -digestalg SHA1 -keystore keys/key.jks app/build/outputs/apk/app-release-unsigned.apk unigrade
