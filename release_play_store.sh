...
if [  "$TRAVIS_BRANCH" = "$DEV_BRANCH" ] || [ "$TRAVIS_BRANCH" = "$MASTER_BRANCH" ]; then
  ...
  if [ "$TRAVIS_PULL_REQUEST" = false ]; then
    ...
    if [ -z "$TRAVIS_TAG" ]; then
      ...
      jarsigner -verbose -sigalg SHA1withRSA -storepass $storepass -keypass $keypass -digestalg SHA1 -keystore keys/keys.jks app/build/outputs/apk/app-release-unsigned.apk unigrade
      ...
      supply run -j keys/api-project-202893591038-55de682e5165.json -p com.unigrade.app -b app/build/outputs/apk/app-release-signed.apk
